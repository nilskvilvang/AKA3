...
```{r}
pacman::p_load(tidyverse, AmesHousing, outliers, EnvStats, caTools, Metrics, psych, tdr, readxl, e1071, caret, plotly, rpart, rpart.plot, knitr, hydroGOF, ggridges, tidymodels, sf, patchwork, RColorBrewer, grid, ragg, ipred, rattle, randomForest, glmnet, datasets, plotrix, ggforce, ROCit, car, class) 
```
...
Kanskje de 5 punktene alle tilhører samme kateogri, i så fall er beslutningen åpenbar om å inkludere det nye punktet i denne kategorien. Dersom vi har punkter fra to eller flere kategorier blant de 5 som er nærmest teller vi bare opp antall punkter av de 5 for hver kategori og legger det nye punktet inn i kateogrien med flest punkter blant de 5.
...
Riktige prediksjoner blir da hhv `r cm6[1]` og `r cm6[4]`) = `r cm6[1] + cm6[4]` av `r sum(cm6)` korrekte prediksjoner og `r cm6[2] + cm6[3]` av `r sum(cm6)` feil.
