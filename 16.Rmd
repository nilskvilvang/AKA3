...
Hensikten med å analysere samfunnet rundt oss er å forstå verden. I dette ligger at vi ønsker å kunne forklare hva som skjer og hvorfor (faktuelt og kaulsalt) *og* predikere hva vi tror kommer til å skje. Forklaring og prediksjon kan ofte stå i litt i motsetning til hverandre. Som vi har nevnt et sted tidligere - alle modeller (av samfunnet) er feil. Men noen modeller er mindre feil enn andre, og noen er også nyttige for oss (selv om vi vet de inneholder feil). En modell som inneholder "alt" er ingen modell, men virkeligheten selv. Altså må vi hele tiden foreta valg når vi lager modeller. En modell som inneholder et svært høyt antall variabler kan være mer nøyaktig, men samtidig veldig vanskelig å forstå. Og en modell som forklarer hva som har skjedd godt trenger ikke å være en modell som predikerer godt hva som kommer til å skje. En modell som passer perfekt til de dataene vi har kan være det vi kaller "overfitted" - altså at modellen er utviklet for å forklare dataene våre for nøyaktig slik at den også forklarer tilfeldige avvik og kanskje også regelrette feil. Et aspekt ved maskinglæringsmodeller er at vi forsøker kontrollere for dette ved at vi først trener en modell på deler av datasettet før vi tester modellens evne til å predikere riktig på en annen del av datasettet. 
...
-   Semi-supervised learning: En form for mellomting mellom supervised og unsupervised, dvs. vi kan ha mindre deler av data med input og output som kan brukes på en større datamengde uten kjent/ønsket output. F.eks. at supervised learning bruker unsupervised learning gjennom såkalt "feature engineering", altså et slags ekteskap mellom supervised og unsupervised learning. En bank kan f.eks. ha flere millioner kunder og hundrevis av millioner av transaksjoner. Banken kan analysere en liten prosent av datasettet (transaksjonene) for å klassifisere dem som bedrageri eller ikke (altså man finner ut om et antall transaksjoner er bedrageri eller ikke, klassifiserer dem, og lager en modell som man deretter kan kjøre på alle andre transaksjoner). Hensikten er å slippe å "manuelt" klassifisere hundrevis av transaksjoner som bedrageri eller ikke. 
-   Reinforcement learning: Modellering gjennom algoritmer der henskten/målsetningen er å maksimere en oppfatning om kumulativ utkomme gjennom belønning eller straff. Reinforcement learning brukes f.eks. til å lære maskiner å spille spill mot mennesker, eller til å generere policies/strategier for å maksimere kost/nytte-vurderinger i ulike sammenhenger. Andre eksempler kan være selvstyrende biler, trading i finans- og investeringsbeslutninger, "natural language processing - NLP" som innebærer automatisk behandling av tekst (f.eks. i oversettelser) og anvendelser i helsesektoren (som "dymanic treatment regimes - DTR") innenfor diagnose og behandling. Modellen utvikles gjennom å straffe "feil" beslutninger/tiltak og belønne "riktige". 

I tillegg skal man ikke holde på lenge med maskinlæring før begrepene Neural networks, Natural Language Processing (NLP) og Deep learning dukker opp. Disse kan, slik vi ser det, alle assosieres med såvel supervised som unsupervised learning. Vi kommer ikke til å gå inn på disse begrepene i dette kapittelet. 
...
For å trene modellen må vi kjøre observasjoner gjennom algoritmen (modellen), gjøre prediksjoner, sjekke med "fasit", evaluere modellen, og gjenta prosessen:
...
Vi skal i dette kapittelet ta utgangspunkt i det opprinnelige datasettet. Det finnes mange versjoner av dette datasettet på nett siden det er et populært datasett, også i konkurranser på nett innen maskinlæring. I enkelte R-pakker - som "tidymodels" - ligger en modifisert versjon av datasettet integrert så man slipper å laste ned datasettet om man bruker denne pakken. 
...
Vi kan følge samme prosedyre som ovenfor for å få fram revidert liste over NAs (viser ikke koden for dette).
...
```{r}
Ames2 <- Ames2 %>% 
  mutate(Lot.Frontage = ifelse(is.na(Lot.Frontage),
                               mean(Lot.Frontage, na.rm = TRUE), Lot.Frontage))
```
...
kable(dimensjoner, caption = "Splitting av datasettet Ames Housing")
```
...
La oss gå tilbake til eksempelet med mtcars og se på kun de to uavhengige variablene. Eksempelet under er kun illustrerende:
...
RMSE er `r round(modelprediksjon, 2)`. Dvs at i gjennomsnitt bommer modellens prediksjoner med \$`r round(modelprediksjon, 2)`.
...
Vi kan se at RMSE er `r round(ames_bag1$err, 2)`.
...
Et annet forhold vi kan legge merke til er at confusion matrix gir oss type I og type II feil (jfr diskusjon i kapittel 2 om disse feiltypene). Vi kan illustrere forholdet mellom ekte og falske positive/negative gjennom en av Æsops kjente fabler - om gjetergutten som ropte Ulv!Ulv!
...
Antallet type I feil (som da er type I feil) er `r cm2[3]` og antall type II feil er `r cm2[4]`. Modellen vår har i dette tilfellet en presisjon på `r round((cm2[1] + cm2[2])/(cm2[1] + cm2[2] + cm2[3] + cm2[4]), 2)` og en feilrate på `r round((cm2[3] + cm2[4])/(cm2[1] + cm2[2] + cm2[3] + cm2[4]), 2)`.

Vi kan legge til at man bør utvise forsiktighet med å kun se på presisjonen til en modell da den i gitte tilfeller kan lure oss ved at presisjonen fkatisk kan være høyere ved å bare anta en verdi i alle tilfeller (altså - ingen modell i det hele tatt). Vi tar for oss dette litt mer i detalj i et delkapittel etter de ulike klassifiseringsalgoritmene.
...
ROC-kurven over vises med indikasjon for det optimale punktet (der avstanden mellom ROC-kurven og den såkalte "chance line" er størst). Dette punktet kalles "Youden Index" [@youdenIndexRatingDiagnostic1950] som er et mål på den mest optimale balansen mellom sensitivitet og spesifisitet.
...
Datasettet består av 569 observasjoner (pasienter). Når vi ser på datasettet som er lastet inn ser vi at en variabel ("X") ikke inneholder data (alle er NA) så denne tar vi bort. Vi sjekker også om datasettet har mangelende verdier (NAs).
...
I vårt tilfelle innebærer våre resultater at modellen har høy nøyaktighet og tilnærmet perfekt kappaverdi.
...
Her vil vi henvise til det første avsnittet i dette kapittelet der vi kort nevnte begrepet "overfitting". Hvis vi ser på de røde feltene i det grønne området i treningssettet over ser vi flere små røde felt som fanger opp noen røde punkter inne på det grønne området. Modellen forsøker altså å forklare så mange observasjoner som mulig og det er tilsynelatende bra fordi vi ønsker jo en modell som er bra. Men når vi ser på neste graf (testsettet) ser vi at vi har disse røde områdene inne på det grønne, men de treffer ikke noen røde punkter. Modellen vi har trent (på treningssettet) ønsker altså å predikere at det bør være noen røde punkter i det grønne området og viser dette med de små røde områdene, men når vi kjører modellen på testsettet er ikke de røde punktene der. Dette er et tegn på "overfitting" - modellen har kanskje i for stor grad forsøkt å tilpasse seg de faktiske dataene i treningssettet. Så vi får en presis modell, men som kanskje predikerer dårligere på nye data enn en litt mindre presis modell. Igjen: dette er grunnen til at vi i maskinlæring først trener en modell og så tester den på nye data som ikke har vært involvert i treningen av modellen. Dataene for trening- og testsett kan (og vil ofte) komme fra samme datasett, men vi deler det altså opp i to før vi starter trening av modellen slik vi har sett gjennom dette kapittelet.
...
Bayesiansk statistikk, i motsetning til "vanlig" frekventistisk statistikk (som vi har drevet med til nå) baserer seg på sannsynligheter, mens frekventistisk statistikk ikke baserer seg på sannsynligheter. Bayesiansk statistikk kalles derfor også probabilistisk statistikk. Hva mener vi med det? @k.hackenbergerBayesNotBayes2019 beskriver dette slik:
...
Vi antar at vi har observasjoner som vi kan dele i to kategorier - de som tar buss og de som tar tog til jobben (her er datapunktene generert for eksempelet og variablene x1 og x2 er registreringer vi har gjort av to ulike karakteristikker ved disse variablene som gjør at vi kan klassifisere et antall pendlere som brukere av enten buss eller tog):

```{r echo = FALSE}
x1 <- c(1, 1.2, 1.3, 1.4, 1.5, 1.4, 1.3, 1.25, 1.15, 1.2, 1.6, 1.7, 1.72, 1.6, 1.7)
x2 <- c(2, 2.2, 2.1, 2.3, 2.25, 2.1, 2, 1.9, 1.9, 2, 2.3, 2.3, 2.4, 2.35, 2.38)
sett1 <- tibble(x1, x2)
...
knnplott <- ggplot() +
    ...
    geom_point(data = sett2, aes(x = x3, y = x4, color = "red")) +
    theme_bw() +
    annotate(geom = "text", x  =1.25, y = 2.4, label = "Tar buss", color = "blue") +
    annotate(geom = "text", x = 1.75, y = 2.6, label = "Tar tog", color = "red") +
    theme(legend.position = "none") 
knnplott
```

Vi får en ny observasjon:

```{r echo = FALSE}

knnplott +
    geom_point(aes(x=1.8, y=2.35), colour="magenta") +
    annotate(geom = "text", x = 1.85, y = 2.3, label = "Nytt punkt", color = "magenta")
```

Bayes teorem gjør at vi kan uttrykke følgende:

$p(buss|x) = \frac{p(x|buss)\ *\ p(buss)}{p(x)}$

der

$p(buss|x)$ = Sannsynligheten for at en person tar buss gitt egenskap x (x = datapunktet som utgjøres av x1 og x2) (dette tallet kalles "posterior probability)

$p(x|buss)$ = Sannsynligheten for at en person har egenskap x gitt at de tar bussen (dette tallet kalles "likelihood")

$p(buss)$ = Sannsynligheten for at en person tar buss (dette tallet kalles "prior probability")

$p(x)$ = Sannsynligheten for at en person har egenskap x (dette tallet "marginal likelihood")

I ord kan vi derfor uttrykke dette slik:

$Posterior\ probability = \frac{likelihood\ *\ prior\ probability}{marginal\ likelihood}$

Vi kan uttrykke det samme for "tog":

$p(tog|x) = \frac{p(x|tog)\ *\ p(tog)}{p(x)}$

Disse sannsynlighetene kan vi regne ut og til slutt sammenlikne sannsynlighetene - hva er sannsynligheten for at en person tar buss gitt egenskap x vs. sannsynligheten for at personen tar tog gitt egenskap x:

$p(buss|x)$ vs. $p(tog|x)$

Forholdet mellom sannsynlighetene avgjør hvilken kategori vi putter det nye datapunktet i. I eksempelet har vi totalt 25 personer som er klassifisert (før det nye punktet legges til). Det gjør at

$p(buss) = \frac{antall\ som\ tar\ buss}{totalt\ antall} = \frac{15}{25} = 0.6$

Dette er med andre ord ganske rett fram. Det blir litt mer kludrete når vi skal regne ut "marginal likelihood". Her vil vi gå tilbake til plottet og legge til en sirkel med en radius vi selv setter rundt det nye punktet:

```{r echo = FALSE}

knnplott +
    geom_point(aes(x=1.8, y=2.35), colour="magenta") +
    geom_circle(aes(x0 = 1.8, y0 = 2.35, r = 0.15), inherit.aes = FALSE)
```

I denne sirkelen - vi skal igjen se bort fra det nye punktet, det er kun der for å definere origo for sirkelen - teller vi opp alle punkter (totalt). Punktene som ligger inne i sirkelen kan sies å ha ganske like egenskaper (x1, x2) som det nye punktet. Vi får da:

$p(x) = \frac{antall\ observasjoner\ i\ sirkelen}{totalt\ antall\ observasjoner} = \frac{4}{25} = 0.16$

Vi kan fortsette med å regne ut "likelihood". Dette er altså sannsynligheten for at en person som tar bussen har egenskap x. Men her bruker vi bare samme plott som ovenfor og finner hvor mange innenfor sirkelen som tar buss og ser det i forhold til hvor mange som tar buss totalt, og får:

$p(x|buss) = \frac{antall\ observasjoner\ i\ sirkelen\ som\ tar\ buss}{totalt\ antall\ som\ tar\ buss} = \frac{3}{15} = 0.2$

Vi kan da sette alt dette sammen: 

$p(buss|x) = \frac{0.2 * 0.6}{0.16} = 0.75$

Det er altså 75 % sannsynlighet for at en person med egenskap x tar bussen. Neste steg er å gjøre akkurat de samme beregningene for $p(tog|x)$ - altså sannsynligheten for at en person med egenskap x (som altså er samme egenskap x som ga 75% sannsynlighet for at personen brukte buss). Dette er 0.25. Siden 0.75 er større enn 0.25 vil vi klassifisere det nye punktet som blått - altså at personen tar buss. 

Vi går videre med eksempelet fra de andre algoritmene.
...
La oss anta at vi har brukt en klassifiseringsalgoritme og fått dette resultatet:

```{r}
Nei <- c(8400, 160)
Ja <- c(180, 20)
cm_acc <- data.frame(Nei, Ja)
rownames(cm_acc) <- c("Nei", "Ja")
cm_acc
```

Her vil vi ha en falsk positiv rate på `r cm_acc[1,2]` og en falsk negativ rate på `r cm_acc[2, 1]`. I dette eksempelet ville vi kunne regne ut presisjonen på modellen vår slik:

$\frac{Antall\ korrekte}{Antall\ totalt} = \frac{8420}{8760} = 0.96$

Dette er jo ganske høy presisjon, men la oss si at vi i stedet for modellen bare predikerte 0 på alle. Da ville confusion matrix sett slik ut:

```{r}
Nei <- c(8560, 200)
Ja <- c(0, 0)
cm_acc2 <- data.frame(Nei, Ja)
rownames(cm_acc2) <- c("Nei", "Ja")
cm_acc2
```

Nå ville preisjonen bli:

$\frac{8560}{8760} = 0.98$

Presisjonen på vår "tilfeldige" modell - der vi bare sier "nei" på alle prediksjoner er altså høyere enn modellen fra klassifiseringsalgoritmen vi brukte. Men intuitivt virker vel dette rart. Og det er det. Vi vil nok helt klart si at modellen fra algoritmen har en høyere prediktiv evne selv om den har lavere presisjon enn den andre fordi den "tilfeldige" modellen rett og slett er for enkel (og "dum"). Typisk vil dette paradokset kunne inntreffe når man har veldig ubalanserte matriser - "class imbalance" - eller skjeve fordelinger mellom klassifiseringene (som f.eks. typisk kan skje om enten 0 eller 1/ja eller nei er sjeldne hendelser). 
