...
### Søylediagram/stolpediagram ("bar chart")
...
En alternativ måte å framstille det samme:
...
Det kan være lett å blande sammen søyle-/stolpediagram og histogram. En visuell huskeregel kan være at et søyle-/stolpediagram har luft (mellomrom) mellom søylene, mens et histogram ikke har det. Dette er en "sannhet" med visse modifikasjoner - man kan få histogrammer med noe mellomrom mellom enkelte søyler, men forskjellen er at i et søyle-/stolpediagram er det mellomrom mellom *alle* søylene. I histogram ligger det store flertallet alltid inntil hverandre uten mellomrom (se eksempler på histogram lenger ned).
...
Kakediagram er generelt ikke anbefalt av mange fordi det oppfattes som vanskelig å se nyanser (det er hevdet at "ekte statistikere bruker ikke kakediagram"...). Se f.eks. på dette eksempelet [@wikipediaPieChart2022]

![Bilde fra Wikipedia](Kakediagram.png){width=75%}

Når man ser på kakene er det ikke umiddelbart lett å se forskjellene mellom de tre, noe vi langt lettere får fram gjennom de tilhørende søylediagrammene.
...
```{r fig.cap = "Densitykurver med gjennomsnittsverdi for to grupper"}
snitt <- hoydefelles %>% 
  group_by(Kjonn) %>%
...
```{r fig.cap = "Densitykurver for tre grupper for en variabel"}
