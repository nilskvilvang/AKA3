...
```{r}
krysstab <- addmargins(table(kji1$Kjønn, kji1$Fargeblind),c(1,2))
krysstab
```
...
```{r}
kjitest1 <- chisq.test(kji1$Kjønn, kji1$Fargeblind)
kjitest1
```
...
```{r}
kjitest2 <- chisq.test(kji1$Kjønn, kji1$Fargeblind, correct = FALSE)
kjitest2
```
...
Hvis kjikvadratverdien er større eller lik den kritiske verdien er resultatene våre statistisk signifikante. Den kritiske verdien i dette eksempelet er 3.84, mens kjikvadratverdien er hhv. 3.75 og 4.43. 
...
```{r}
test <- fisher.test(table(kji1$Kjønn, kji1$Fargeblind))
test
```
...
```{r}
krysstab2 <- table(kji1$Kjønn, kji1$Fargeblind) 
...
phikoeff <- data.frame(r = c(".70 eller høyere", "0.40 - 0.69", "0.30 - 0.39", "0.20 - 0.29", "0.01 - 0.19", "0"),
...
Differansen mellom 4.43 og 4.42 sjykdes avrundinger.
