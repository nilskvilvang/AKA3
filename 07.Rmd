...
```{r fig.cap = "Deskriptiv statistikk for data til ANOVA"}
...
```{r warning = FALSE, fig.cap = "Boxplot for tre variabler"}
...
Siden p-verdien er `r format(round(summary(resultataov1)[[1]][["Pr(>F)"]][1], 3), nsmall = 3)` kan vi konkludere med at det er signifikante forskjell et sted (mellom to eller flere grupper) i variabelen (agegp3). Men vi kan ikke ut fra dette si hvor – altså hvilken gruppe som er signifikant forskjellig fra de andre). 
...
```{r fig.cap = "Shapiros test"}
shapirotest <- shapiro.test(optimisme$toptim)
shapirotest
```

Siden testverdien er `r shapirotest$p.value` (< 0.05) må vi anta at dataene er signifikant forskjellig fra normalfordelingen. Dette betyr at vi bør bruke Levenes og/eller Fligner-Killeen. 

```{r fig.cap = "Levenes test"}
...
```{r fig.cap = "Fligner-Killeen test"}
flignertest <- fligner.test(toptim ~ agegp3, data = optimisme)
flignertest
```

```{r echo = FALSE}
pverdi <- round(leveneoptimisme$`Pr(>F)`[1], 2)
```


Tolkningen av alle tre testene for homogenitet i varians er den samme: Hvis p < 0.05 er variansen ikke lik. Det fremheves imidlertid at dersom gruppene er tilnærmet like store er ikke denne forutsetningen kritisk (eller sågar nødvendig) - ANOVA (og t-tester) er generelt robuste i forhold til brudd på forutsetningen om homogen varians dersom gruppene er relativt like [@statisticssolutionsAssumptionHomogeneityVariance2013]. I vårt eksempel viser f.eks. Levenes test (p = `r pverdi`) at vi har homogenitet i variansen.

Et alternativ er å gjøre en såkalt Welch enveis test. Welch test forutsetter ikke homogen varians:

```{r fig.cap = "Welch's test"}
...
```{r fig.cap = "TukeyHSD post-hoc test"}
...
```{r fig.cap = "Reorganisering av format til ¨long' format"}
...
```{r fig.cap = "Deskriptiv statistikk for datasett for enveis ANOVA"}
selvtillit3 %>%
  group_by(time) %>%
...
```{r warning = FALSE, fig.cap = "Boxplott for tre variabler"}
ggboxplot(
  selvtillit3, 
  x = "time", 
  y = "score",
...
```{r fig.cap = "Shapiro-test for tre variabler"}
selvtillit3 %>%
  group_by(time) %>%
  shapiro_test(score)
```
...
```{r fig.cap = "Sjekk av uteliggere grafisk"}
...
Siden vi ser at p = `r round(res$p, 3)` (< 0.05) for Mauchly's test bruker vi nederste del av resultatet ovenfor. Det er to tester som vises der som begge gjør korreksjoner på antall frihetsgrader gitt at forutsetningen om sferisitet ikke er oppfyllt. Vi går ikke inn på hvordan disse korreksjonene gjøres, ut over F verdiene (= forholdet mellom systematisk og usystematisk varians) vil være like for alle tre, men at signifikansverdien vil endres ettersom det er den kritiske verdien gjennom frihetsgradene som endres ved de to korreksjonene). 
```{r echo = FALSE}
epsilonverdi <- resultataov4$`Sphericity Corrections`
```

1. Greenhouse-Geisser (GG) som gir $\epsilon$ (epsilon) - som gir en verdi mellom 0 og 1 på hvor langt de foreliggende dataene er fra det ideelle/optimale. I vårt tilfelle er epsilon = `r round(epsilonverdi$GGe, 3)` (se "GGe" i resultatet ovenfor). @karadimitriouRepeatedMeasuresWithinsubjects angir at man bør bruke Huynh-Feldt dersom $\epsilon > 0.75$ eller om man har små utvalg (10). Her kan vi derfor se på verdien i p[GG] som er < 0.005. Dette indikerer at det er en signifikant effekt/forskjell et sted mellom måletidspunktene. 
...
```{r echo = FALSE}
resultataov4apa <- apa.ezANOVA.table(resultataov4, correction = "GG", table.title = "", table.number = NA)
resultataov4apa
```

Vi ønsker da å se på de parvise sammenlikningene for å se hvor vi har signifikante forskjeller:

```{r fig.cap = "Parvise sammenlikninger"}
padj <- selvtillit3 %>%
  pairwise_t_test(
...
```{r fig.cap = "p adjusted"}
padj$p.adj
```

Vi kan se at det er statistisk signifikant forskjell mellom alle tre parene: confid1 - confid2 = `r format(round(padj$p[1], 3), nsmall = 3)`, confid1 - confid3 = `r format(round(padj$p[2], 3), nsmall = 3)` og confid2 - confid3 (`r format(round(padj$p[3], 3), nsmall = 3)`) (p er selvagt ikke 0, men 0.000 angir f.eks. for siste par `r padj$p[3]`).

Til slutt ser vi på effektstørrelsen ($\eta^2$). Denne ble gitt automatisk da vi brukte pakken "ez" som "ges" ("ges" = Generalized Eta-Squared). Verdien for "ges" i tabellen over = `r format(round(res$ges, 2), nsmall = 2)`. Vi kan også hente verdien direkte (funksjonen "eta-squared" klarer ikke å hente fra ezANOVA, men som sagt ligger det som "ges" i output fra ezANOVA):

```{r fig.cap = "Eta squared direkte fra aov"}
...
```{r fig.cap = "Omega squared direkte fra aov"}
...
```{r fig.cap = "Tolkning av omega squared"}
...
Vi ser at Cohens d for tid 1 - tid 2 er `r format(round(cohen1$estimate, 3), nsmall = 3)` og for tid 2 til tid 3 er `r format(round(cohen2$estimate, 3), nsmall = 3)`. Dette gir følgende tolkning [@ben-shacharEffectsizeEstimationEffect2020a], jfr. @gignacEffectSizeGuidelines2016:

```{r fig.cap = "Tolkning Cohens d"}
...
```{r fig.cap = "Oppsummering nnveis avhengig ANOVA"}
...
```{r fig.cap = "Grafisk framstilling tre variabler til toveis ANOVA"}
ggpubr::ggboxplot(optimisme2, x = "agegp3", y = "toptim", color = "sex", palette = c("#00AFBB", "#E7B800"))
```

```{r fig.cap = "Toveis ANOVA"}
...
Videre ser vi på effektstørrelsen. For alder, som var signifikant, finner vi en liten effekt (Partial Eta Squared = `r resultataov6[2, 7]` (under kolonnen "ges"), hvilket er en middels effekt - jfr. tidligere grenseverdier gitt av @cohenStatisticalPowerAnalysis1988.

Forutsetning om homogenitet i varians:
```{r fig.cap = "Forutsetning om homogenitet i varians"}
...
```{r echo = FALSE}
writexl::write_xlsx(statfrykt, "experim3.xlsx")
```
...
```{r fig.cap = "Innlesing av data"}
statfrykt <- read_excel("experim3.xlsx")
head(statfryktlong)
```

```{r fig.cap = "ANOVA-modell"}
statfryktaov <- aov_ez(
  id = "id",
  dv = "score",
  data = statfryktlong,
...
```{r fig.cap = "APA-tabell for ANOVA"}
apatabell <- ezANOVA(
    statfryktlong, 
    dv = .(score),
    wid = .(id),
...
```{r warning = FALSE, message = FALSE, fig.cap = "ANOVA-plott"}
...
```{r fig.cap = "Effekstørrelse omega squared"}
effektstorrelse <- omega_squared(statfryktaov)
```

```{r fig.cap = "Tolkning omega squared"}
...
Vi ser at Cohens d for tid 1 - tid 2 er `r format(round(cohen3$estimate, 2), nsmall = 2)` og for tid 2 til tid 3 er `r format(round(cohen4$estimate, 2), nsmall = 2)`. Dette gir følgende tolkning [@ben-shacharEffectsizeEstimationEffect2020a], jfr. @gignacEffectSizeGuidelines2016:

```{r fig.cap = "Tolkning av effekter"}
