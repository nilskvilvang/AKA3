...
```{r}
pacman::p_load(tidyverse, AmesHousing, outliers, EnvStats, caTools, Metrics, psych, tdr, readxl, e1071, caret, plotly, rpart, rpart.plot, knitr, hydroGOF, ggridges, tidymodels, sf, patchwork, RColorBrewer, grid, ragg, ipred, rattle, randomForest, glmnet) 
```
...
I kapittelet om regresjonsanalyse (ikke medregnet logistisk regresjon) har vi brukt minste kvadratsums metode ("Ordinary Least Squares"). Dersom vi har mange forklaringsvariabler i forhold til antall observasjoner eller stor korrelasjon mellom forklaringsvariablene (multikolinearitet) kan det være aktuelt å bruke alternative metoder fordi minste kvadratsums metode vil medføre stor varians i estimatorene. En av disse er "ridge regression" [@hoerlRidgeRegressionApplications; @hoerlRidgeRegressionBiased]. Vi tenker det er i overkant teknisk å gå inn i utregning (se f.eks. [her](https://www.publichealth.columbia.edu/research/population-health-methods/ridge-regression) for en rimelig bra forklaring (på engelsk), men poenget er å få estimatorer som er mer "korrekt" under de betingelsene som er nevnt over. Ridge regresjon utvider lineær regresjon ved å ta hensyn til kompleksiteten i modellen ved å introdusere en "straffeparameter" ("penalty parameter") som utgjøres av den kvadrerte størrelsen av koeffisientene.   
...
```{r}
y <- data.matrix(mtcars[mtcars$hp],)
y
```
...
Vi kommenterer ikke modellen på dette stadiet. Neste steg blir å velge en såkalt lambdaverdi ($\lambda$) som gir lavest MSE (Mean Square Error).

```{r}
cv_ridgemodell <- cv.glmnet(x, y, alpha = 0)
beste_lambda <- cv_ridgemodell$lambda.min
beste_lambda
plot(cv_ridgemodell)
```
...
```{r}
beste_modell <- glmnet(x, y, alpha = 0, lambda = beste_lambda)
coef(beste_modell)
```

Et interessant plott er et "trace-plott" som viser hgvordan koeffisientene endrer seg som følge av endret lamda-verdi:
...
```{r}
y_predicted <- predict(ridgemodell, s = beste_lambda, newx = x)
sst <- sum((y - mean(y))^2)
